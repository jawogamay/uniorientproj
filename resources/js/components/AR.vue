<template>
  ...
  <tr style="height:30px;background:#000;">
    ...
    <td class="text-xs-left"><button class="btn btn-success --primary" @click="viewPassenger(props.item)">VIEW <i class="fa fa-eye"></i></button>
        ...
        <td class="text-xs-left">{{ props.item.startar | capitalize }}</td>
                ...
                <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="addNewLabel">ADD AR START SERIES</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                            </button>   
                        </div>
                       <form @submit.prevent="createStartAR()">
                        <div class="modal-body">
                          <input class="form-control" type="number" style="width:100%;" name="start" v-model="form.start" placeholder="ENTER STARTING AR SERIES">
                        </div>
...
<script type="text/javascript">
    export default{
        ...
        headers: [
          ...
          { text: 'START AR', value: 'startpo',sortable: !1},
        ...
        methods: {
            ...
            createStartAR(){
                this.form.post('api/ar')
                .then((response)=>{
                  this.spinner = true;
                  Fire.$emit('createdStartAR')
                  $('#addNew').modal('hide');
                  toast.fire({
                    type: 'success',
                    title: 'Start AR Series Created Successfully'
                  });
            ...
            getPO(){
              axios.get('api/ar').then(({data}) => this.startar = data)
            },
            createdStartAR(){
              this.getPO()
              Fire.$on('createdStartAR',()=>{
                this.getPO()
              })
            },
         /*   viewEmployee(item){
