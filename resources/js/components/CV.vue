<template>
  ...
  <tr style="height:30px;background:#000;">
    ...
    <td class="text-xs-left"><button class="btn btn-success --primary" @click="viewPassenger(props.item)">VIEW <i class="fa fa-eye"></i></button>
        ...
        <td class="text-xs-left">{{ props.item.startcv | capitalize }}</td>
                ...
                <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="addNewLabel">ADD CV SERIES</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                            </button>   
                        </div>
                       <form @submit.prevent="createStartCV()">
...
<script type="text/javascript">
    export default{
        ...
        headers: [
          ...
          { text: 'START CV', value: 'startcv',sortable: !1},
        ...
        methods: {
            ...
            createStartCV(){
                this.form.post('api/cv')
                .then((response)=>{
                  this.spinner = true;
                  Fire.$emit('createdStartCV')
                  $('#addNew').modal('hide');
                  toast.fire({
                    type: 'success',
            ...
            getCV(){
              axios.get('api/cv').then(({data}) => this.startcv = data)
            },
            createdStartCV(){
              this.getCV()
              Fire.$on('createdStartCV',()=>{
                this.getCV()
              })
            },
         /*   viewEmployee(item){
